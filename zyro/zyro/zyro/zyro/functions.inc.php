...
function parse_uri(SiteInfo $siteInfo) {
	...
	$lang = ($siteInfo->defLang ? $siteInfo->defLang : null);
	...
	if (!$ru_) {
		foreach ($siteInfo->pages as $idx => $pi) {
			if ($siteInfo->homePageId == $pi['id']) return array($idx, $lang, $ru, null);
		}
...
function getCurrUrl($cutProto = false, $cutQuery = false, $port = null) {
	$isHttps = isHttps();
	list($host) = explode(':', $_SERVER['HTTP_HOST'], 2);
	$url = ($cutProto ? '' : ($isHttps ? 'https' : 'http').'://').$host;
